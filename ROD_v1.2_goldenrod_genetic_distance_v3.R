...
ROD_v1.2 <- read_rds("./../04_github/ROD/ROD_v1.2_operon_variants.rds") %>% as_tibble()
ROD_v1.2_genome_stats <- read_rds("./../04_github/ROD/ROD_v1.2_genome_statistics.rds") %>% as_tibble()
# saveRDS(ROD_v1.2_genome_stats,"./../04_github/ROD/ROD_v1.2_genome_stats.rds")
head(ROD_v1.2$assembly_id)
...
# multicopy <- ROD_v1.2_genome_stats %>% filter(rDNA_variants > 1)
# assembly_id_list <- multicopy$assembly_id
# ROD_v1.2 %>% filter(assembly_id %in% assembly_id_list)
...
all_alignments <- sort(list.files("./../05_goldenROD/pr_subdivision/mafft_big/", full.names = TRUE))
all_alignments <- sort(list.files("./../05_goldenROD/pr_subdivision/mafft/", full.names = TRUE))
all_alignments <- sort(list.files("./../05_goldenROD/pr_subdivision/mafft_ROD_v1.2/", full.names = TRUE))

# TESTING one by one: 
alignment <- readDNAMultipleAlignment(all_alignments[1])
basename(all_alignments[1]) %>% str_remove(".mafft.fasta") %>% str_remove("ROD_v1.2_")
...
for(alignment in all_alignments[2]) {
  ...
  distances <- dist.dna(alignment_ape, model = "TN93", as.matrix = T)
  weighted_distance <- calculate_weighted_distances(distances)
  alignment_name <- basename(alignment) %>% str_remove(".mafft.fasta") %>% str_remove("ROD_v1.2_")
