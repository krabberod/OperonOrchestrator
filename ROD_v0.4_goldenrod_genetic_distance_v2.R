...
library(ape)
library(msa)
source("calculate_weighted_distances.R")
...
basename(all_alignments[2]) %>% str_remove(".mafft.fasta") %>% str_remove("ROD_v0.4_")
...
for(alignment in all_alignments) {
  ...
  weighted_distance <- calculate_weighted_distances(distances)
  alignment_name <- basename(alignment) %>% str_remove(".mafft.fasta") %>% str_remove("ROD_v0.4_")
...
ROD_v0.4_genome_stats <- ROD_v0.4_genome_stats %>% select(-colnames(ROD_v0.4_genome_stats)[25:30])
ROD_v0.4_genome_stats <- left_join(ROD_v0.4_genome_stats, summary_stats)
ROD_v0.4_genome_stats$std_dev
...
ggplot(summary_stats, aes(x = alignment, y = mean_distance)) +
  geom_boxplot() +
...
library(pheatmap)
pheatmap(distance_matrices_weighted[[234]], color = colorRampPalette(c("blue", "white", "red"))(50), 
         clustering_distance_rows = "euclidean", clustering_distance_cols = "euclidean",
         main = "Genetic Distance Heatmap for Species 1")

# Bar chart for comparing mean distances across species
ggplot(summary_stats, aes(x = alignment, y = mean_distance, fill = alignment)) +
...
ggplot(summary_stats, aes(x = distance_range, y = mean_distance)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, ),legend.position = "none") +
  labs(title = "Genetic Distance vs. Habitat Diversity", x = "distance_range", y = "Mean Genetic Distance")
